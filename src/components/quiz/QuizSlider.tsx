...
const QuizSlider: React.FC<QuizSliderProps> = ({ options, selectedValue, onSelect }) => {
  ...
  return (
    <div className="w-full">
        ...
        <div className="relative flex justify-between">
          {options.map((option, index) => (
            <button
              ...
              style={{ left: `calc(${(index / (options.length - 1)) * 100}% - ${index === 0 ? 0 : index === options.length - 1 ? (isMobile ? 16 : 8) : (isMobile ? 16 : 8) / 2}px)` }}
            />
          ))}
        </div>
      </div>
      ...
      {/* Option labels */}
      <div className={`flex justify-between ${isMobile ? "mt-14" : "mt-6"}`}>
        {options.map((option) => (
          <button
            key={option.value}
            onClick={() => onSelect(option.value, option.points)}
            className={`py-3 px-2 rounded-lg text-center ${isMobile ? "w-[30%]" : ""} ${
              selectedValue === option.value
